...
> **Quad-consumer policy (B5, 2026-09-24)**: every new example band delivers FOUR consumers of one artifact —
> (1) example source, (2) a row in this index with prose, (3) a CI-runnable test (ctest / smoke argv),
> (4) a gallery card (auto-generated by `pixi run gallery` from the same registries).
> Machine lock: gate-docs check ⑥ (gallery manifest == disk registry set). Adding an example without
> registering it breaks the build (R9 anti-glob); forgetting the gallery run breaks gate-docs.
