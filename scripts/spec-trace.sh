...
sdds=$(grep -ohE '^## (CORE|REND|WGPU|GLTF|GEO|CAPI)-[0-9]{2}\b' docs/sdd/*.md 2>/dev/null | sed 's/^## //' | sort -u)
tags=$(grep -rhoE '// spec: (CORE|REND|WGPU|GLTF|GEO|CAPI)-[0-9]{2}' crates/ bindings/ 2>/dev/null | grep -oE '(CORE|REND|WGPU|GLTF|GEO|CAPI)-[0-9]{2}' | sort -u)
