...
fn persp_center_and_corner_rays() {
    ...
    assert!((c.dir.x / -c.dir.z + tx).abs() < 1e-9 && (c.dir.y / -c.dir.z - ty).abs() < 1e-9);
    // 右下角：镜像
    let d = screen_to_ray_persp(&rig, W, H, W, H).unwrap();
    assert!((d.dir.x / -d.dir.z - tx).abs() < 1e-9 && (d.dir.y / -d.dir.z + ty).abs() < 1e-9);
    // 归一化
    let n = (d.dir[0] * d.dir[0] + d.dir[1] * d.dir[1] + d.dir[2] * d.dir[2]).sqrt();
...
fn ortho_and_d7_far_field() {
    ...
    assert!(r.dir[0].abs() < 1e-9 && r.dir[1].abs() < 1e-9 && (r.dir[2] + 1.0).abs() < 1e-9);
