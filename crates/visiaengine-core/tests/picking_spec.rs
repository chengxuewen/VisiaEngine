...
use visiaengine_core::{Ray, ray_aabb, ray_triangle, ray_triangle_cw};

const Z5: [f64; 3] = [0.0, 0.0, 5.0];
...
fn ray_triangle_hit_miss_and_backface() {
    ...
    // 强制双面口（marker 拾取用）
    assert!(ray_triangle_cw(r, a, c, b).is_some());
    // 平行 / 射线背后三角形 / 出界命中（穿越但 u+v>1）
    let par = Ray { origin: Z5, dir: [1.0, 0.0, 0.0] };
    assert!(ray_triangle(par, a, b, c).is_none());
    let behind = Ray { origin: [0.0, 0.0, -1.0], dir: [0.0, 0.0, -1.0] };
    assert!(ray_triangle(behind, a, b, c).is_none());
    let edge = Ray { origin: [3.0, 3.0, 5.0], dir: [0.0, 0.0, -1.0] };
...
#[test]
fn ray_aabb_prune() {
    let r = Ray { origin: Z5, dir: [0.0, 0.0, -1.0] };
    ...
    assert!(ray_aabb(r, [0.5, 0.5, 4.0], [1.0, 1.0, 6.0]), "起点在框内=命中");
}
